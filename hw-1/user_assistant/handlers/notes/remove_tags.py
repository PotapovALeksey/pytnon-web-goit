...
class NotesRemoveTagsHandler(NotesAbstract):
    def execute(self):
        ...
        Console.print_table(
            f"Note updated with removed tags", note_titles, [get_notes_row(note)]
        )
        while True:
            tags_input = (
                input_value("selected tags to remove (separate by comma)", str, True)
                .strip()
                .casefold()
            )
            ...
            if tags_input:
                tags_to_remove = set(
                    filter(lambda tag: len(tag) > 0, tags_input.split(","))
                )
                ...
                if not set(note.str_tags).issuperset(tags_to_remove):
                    ...
                    return Console.print_table(
                        f"Note updated with removed tags",
                        note_titles,
                        [get_notes_row(note)],
                    )
